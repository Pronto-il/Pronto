...
- `GET /api/availability/blocks/{blockId}` — reads one of the caller's own blocks, same
  authorization sequence as edit (`404 NOT_FOUND` → `403 FORBIDDEN`), same `BlockResponse`
  body. Exists because the calendar's `BLOCKED` segments are *derived*: a block is clipped to
  each day's working-hours window, so a multi-day block reaches a client as several day-sized
  segments sharing one `blockId`. An editor seeded from a clicked segment would therefore
  `PATCH` a multi-day block down to that one day — the frontend block editor loads the real
  row through this endpoint first. A single-id read, not the range listing the calendar
  section below deliberately does not provide.
...
| `controller.AvailabilityController` | `/api/availability/slots` (`POST`), `/api/availability/slots/me` (`GET`), `/api/availability/slots/{slotId}` (`PUT`, `DELETE`, new Milestone 7), and, as of Milestone 4, `/api/availability/sos-availability` (`PUT`, `GET`). **As of M1**, also `/api/availability/working-hours` (`GET`, `PUT`), `/api/availability/blocks` (`POST`), `/api/availability/blocks/{blockId}` (`GET`, `PATCH`, `DELETE`), and `/api/availability/calendar` (`GET`, `from`/`to` bound as raw `String` query params rather than `Instant`, so a parse failure is guaranteed to surface as `400 VALIDATION_ERROR` via this class's own explicit handling rather than depending on Spring's default query-param type-conversion failure path, which this codebase's `GlobalExceptionHandler` has no dedicated handler for). |
