...
/**
 * Whether entering `state` should fire the one-shot arrival celebration.
 *
 * Both `ARRIVED` and `REVIEW` qualify, and the second one is not redundant: `GET
 * /api/bookings/orders/me` is polled, so a short visit can go `ON_THE_WAY -> COMPLETED` between
 * two ticks and the client never observes `ARRIVED` at all. Without the `REVIEW` case those
 * customers would simply never see the animation. The caller keys it by order id so it still
 * plays at most once per order.
 */
export function isCelebrationState(state: ToolboxState): state is Extract<ToolboxState, { kind: 'ARRIVED' | 'REVIEW' }> {
  return state.kind === 'ARRIVED' || state.kind === 'REVIEW';
}
