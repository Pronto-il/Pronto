...
## Status
**Standard flow implemented, Frontend Milestone 3 (2026-08-16); post-QA bug-fix pass
(2026-08-17).** SOS flow is still not built (Milestone 4 frontend scope).
...
Not built here: SOS request handling, job-status action buttons beyond cancel (on-the-way/
complete are professional-only and belong to a future professional job-status screen, not
this milestone's `features/dashboard` scope either), slot edit/delete UI, favorites
toggle interaction, reviews UI.
